...
Starting the tidy phase before v1.0.0

### Added
- `Alien::Xrepo::Base::Builder` installs every package returned by the alien class's `package_names` method (single-package classes keep the old behavior) and, when one fails to install, records it in ConfigData as `{ error => ... }` instead of aborting the build so consumers can detect unavailable ffi libraries via the new `package_error` and `missing_packages` accessors.
