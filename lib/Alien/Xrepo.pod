...
instance is confined to its own project-local directory instead of the shared per-user store. Used by
alien consumers such as L<Alien::Xrepo::Runtime> and the L<Alien::Xrepo::Build> engine to keep a distribution's
native dependencies inside its C<share> directory.
