...
Optional xmake output theme, passed to xmake as C<$ENV{XMAKE_THEME}>. Defaults to C<plain>, which suppresses ANSI
color codes from xmake/xrepo output (clean CPAN test reports, CI logs, and captures). Set C<theme =E<gt> 'default'>
to restore xmake's colored output. May be overridden per-call with C<theme =E<gt> ...> on any store-touching method.
