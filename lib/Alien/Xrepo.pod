...
This module acts as an intelligent bridge between Perl and the C<xrepo> package manager. With a single C<install> you
can fetch B<shared libraries> as well as B<binary tools and interpreters> without touching a system package manager:
...
or L<Inline::C> to bind the libraries, and C<bin_dir>/C<installdir> to locate and run the binaries.

While L<Affix> or L<FFI::Platypus> or L<Inline::C> can handle the binding or linking to native functions, Alien::Xrepo
handles the B<acquisition> of the libraries and binaries. It automates the entire dependency lifecycle:
...
I<The underlying manager (vcpkg, conan, brew, ...) must be installed and reachable on PATH for the namespace to work.>
...
C<xrepo> does not build only shared libraries - it also ships ready-to-run B<binaries> (C<ninja>, C<cmake>, C<meson>,
