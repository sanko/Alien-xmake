...
Optional configuration options passed to C<xrepo> (shared by most other methods and missing options are filled in automatically):
...
C<install( ... )> downloads and builds the library for you, then C<libpath> points L<Affix> or L<FFI::Platypus> to the matching shared
C<.dll> / C<.so> / C<.dylib>.
