...
L<Capture::Tiny>) so an install that would normally prompt never hangs waiting on C<STDIN>. C<confirm =E<gt> ...>
takes precedence when both are set.

=item B<confirm>

Constructor option. Supplies an explicit answer (C<yes>, C<no>, or C<def>) to any prompt xmake asks, passed through as
C<--confirm=...>. Takes precedence over C<yes =E<gt> 1>. May be terse:
C<Alien::Xmake-E<gt>new( confirm =E<gt> 'yes' )>.
