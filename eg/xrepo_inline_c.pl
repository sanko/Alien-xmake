...
#
# Requires: cpanm Inline::C   (any C compiler works; win32: Strawberry Perl)
...
const char* xrepo_versions() {
    static char buf[128];
    snprintf(buf, sizeof buf, "libpng %s / zlib %s",
...
# Note: avoid a bare "(void)" parameter list in an Inline::C function
# (its parser mis-tokenizes that signature).
say xrepo_versions();