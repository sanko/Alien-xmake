...
# xrepo does not only build shared libraries - it also ships ready-to-run
# BINARIES (ninja, cmake, meson, python, node, go, rust, ...). `install`
# returns the package installdir, and `bin_dir` tells you where the
# executables live.
...
# Option B: put bin_dir first on PATH, so a plain 'ninja' resolves (and every
# child of this process inherits it)
